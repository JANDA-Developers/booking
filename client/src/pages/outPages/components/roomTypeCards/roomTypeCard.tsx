...
const RoomTypeCard: React.SFC<IProps> = ({
  ...
  const countOpFn = (key: "maleCount" | "femaleCount" | "roomCount") => {
    if (key === "maleCount" && availableCount.maleCount) {
      const tempArray: IselectedOption<number>[] = Array(
        availableCount.maleCount.availablePeopleCount.countMale
      )
        .fill(0)
        .map((value, index) => ({
          label: `${index}명`,
    ...
    if (key === "femaleCount" && availableCount.femaleCount) {
      const tempArray: IselectedOption<number>[] = Array(
        availableCount.femaleCount.availablePeopleCount.countFemale
      )
        .fill(0)
        .map((value, index) => ({
          label: `${index}명`,
