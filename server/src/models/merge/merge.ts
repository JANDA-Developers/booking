...
export const extractHouse = async (
  ...
  try {
    ...
    return {
      ...
      product: await transformProduct.bind(transformProduct, house.product),
      smsInfo: transformSmsInfo.bind(transformSmsInfo, house.smsInfo),
      roomTypes: transformRoomTypes.bind(transformRoomTypes, house.roomTypes)
    };
  } catch (error) {
    throw error;
  }
...
export const extractRoomType = async (
  ...
  return {
    ...
    rooms: await transformRooms.bind(transformRooms, extractResult._doc.rooms),
...
export const transformRoom = async (
  ...
  const roomSchema: InstanceType<RoomSchema> | null = await RoomModel.findById(
    roomId
  );
  if (roomSchema) {
...
export const extractSeason = async (
  ...
  return {
    ...
    house: await transformHouse.bind(transformHouse, extractResult._doc.house)
  };
};
...
export const transformSeason = async (
  ...
  const season: InstanceType<SeasonSchema> | null = await SeasonModel.findById(
    seasonId
  );
  if (season) {
    return await extractSeason(season);
...
export const transformSeasons = async (
  ...
  try {
    const seasons: Array<InstanceType<SeasonSchema>> = await SeasonModel.find({
      _id: {$in: seasonIds}
    });
...
export const extractSeasonPrice = async (
  ...
  return {
    ...
    roomType: await transformRoomType.bind(transformRoomType, sp._doc.roomType),
...
export const transformSeasonPrice = async (
  ...
  try {
    ...
    if (seasonPriceId) {
      existingSeasonPrice = await SeasonPriceModel.findById(seasonPriceId);
    } else if (args) {
...
export const extractbooking = async (
  ...
  return {
    ...
    guests: await transformGuests.bind(transformGuests, bookingInstance.guests)
  };
};
...
export const extractGuest = async (
  ...
  return {
    ...temp._doc,
    _id: guest._id,
    booking: await transformbooking.bind(transformbooking, temp._doc.booking),
...
export const extractBlocks = async (
  ...
  return await Promise.all(
    blockInstances.map(async blockInstance => await extractBlock(blockInstance))
  );
};
