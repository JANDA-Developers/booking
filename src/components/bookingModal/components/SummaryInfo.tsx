...
const DefaultCreateKeys: TinputKeys[] = [ 'name', 'phoneNumber', 'funnel', 'checkInOut', 'breakfast' ];
...
const setSetting = (isCreateMode: boolean, data: TinputKeys[]) => {
	try {
		localStorage.setItem(isCreateMode ? createSettingKey : settingKey, JSON.stringify(data));
...
export const SummaryInfo: React.FC<IProps> = ({ bookingModalContext }) => {
	...
	const [ renderInputKeys, setRenderInputKeys ] = useState<TinputKeys[]>(getSetting(isCreateMode));
		...
		<div id="SummaryInfo">
			<JDtypho>
				...
				>
					...
					<Wrap order={funnel}>
						<JDselect options={FUNNELS_OP} label={LANG('funnels')} {...funnelStatusHook} />
					</Wrap>
					<Wrap order={memo}>
					...
					<Wrap>
						<InputText
							readOnly
							id="ResvDateInput"
							value={moment(createdAt || undefined).local().format(DateFormat.WITH_TIME)}
