...
import { BOOKING_STATUS_OP } from '../../../types/const';
...
const ResvInfo: React.FC<IProps> = ({ bookingModalContext, responseStyle, ...prop }) => {
	const { resvDateHook, isDesktopUp, bookingStatusHook, bookingData } = bookingModalContext;
	const { createdAt } = bookingData;
	// smsIcon 핸들

	return (
		<Align {...responseStyle}>
			{isDesktopUp && <JDtypho mb="normal">{LANG('reservation_information')}</JDtypho>}
			<div>
				<JDselect
					mr={'no'}
					{...bookingStatusHook}
