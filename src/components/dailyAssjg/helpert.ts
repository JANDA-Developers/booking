...
export const dateFromTo = (checkIn: any, checkOut: any) =>
  dayjs(checkIn).format(`MM/DD`) + "~" + dayjs(checkOut).format(`MM/DD`);
...
export const checkIsFull = (
  ...
  pricingType === PricingType.DOMITORY
    ? itemLegnth === peopleMax
    : itemLegnth > 1;

// 더블 어레이 리턴
export const getPlaceArray = (
  pricingType: PricingType,
  itemsInRoom: (IG | IB | null)[],
  peopleCountMax: number
) => {
  // 아이템들이 들어갈 자리를 생성
  const places = new Array(
    pricingType === PricingType.DOMITORY ? peopleCountMax : 1
  ).fill([null]);
