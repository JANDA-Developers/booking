...
export const DragBoxPlace: React.FC<IDragBoxPlace> = ({ room, onDrop, itemsInPlace, isFull, roomType, place }) => {
	...
	const dragItemManufacter = (item: IG | IB | null): (IG & IDragItemProp) | (IB & IDragItemProp) | null => {
		if (!item) return null;
		if (instanceOfA<IG>(item, 'roomType')) return Object.assign({ type: item.roomType._id }, item);
	...
	return (
		<div className={classes} ref={drop}>
			{itemsInPlace.map((item, i) => {
				return (
					<DragItem
						key={item === null ? room._id + i + 'empty' : item._id}
