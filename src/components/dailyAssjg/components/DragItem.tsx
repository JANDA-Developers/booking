...
const Wrap = React.forwardRef<HTMLDivElement, IWrapProp>(({ children, className, ...props }, ref) => {
	return (
		<div ref={ref} {...props} className={`dailyAssigItem__itemBlockWrap ${className}`}>
			{children}
		</div>
	);
});
...
const DragItem: React.FC<IProps> = ({ item, place, room, roomType }) => {
	...
	const guestBlockClasses = classNames('dailyAssigItem__itemBlock dailyAssigItem__guest', undefined, {
		'dailyAssigItem__guest--checkIn': checkInInfo.isIn
	});

	return (
		<Wrap ref={drag} data-place="top" data-tip={dateFromTo(checkIn, checkOut)} data-for={'guestCheckInOutToolTip'}>
