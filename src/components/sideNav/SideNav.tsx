...
import JDmenuLinker, { IMenusItem } from '../../atoms/menu/components/MenuLiLinker';
	...
	return (
		<Fragment>
			<div className={classes}>
							...
							{sortedMenus.map(
								(menu) =>
									...
									instanceOfA<IMenusGroup>(menu, 'contents') ? (
										<JDsubMenu
											key={menu.key}
											title={<JDmenuTitle title={menu.groupTitle} className="JDsideNav__menus--pr" icon={menu.icon} />}
										>
											<JDmenuItem title={menu.groupTitle} key={menu.key}>
												<div className="JDsideNav__menus-subItemWrap">
													{menu.contents.map((menu) => (
														<JDmenuLinker
															key={menu.key}
															className={`JDsideNav__menus JDsideNav__menus--sub ${menu.disabled
										...
										<JDmenuItem key={menu.key}>
											<JDmenuLinker
												className={`JDsideNav__menus JDsideNav__menus--sub ${menu.disabled ? 'JDsideNav__navLink--disabled' : ''}`}
												menu={menu}
											/>
										</JDmenuItem>
									)
					...
					<div className="JDsideNav__productView">
						<div className="JDsideNav__billing-info">
							<div className="JDsideNav__billing-title">
								<span className="JDstandard-small-space">{inOr(applyedProduct, 'name', LANG('unapplied'))}</span>
								{applyedProduct && (
									<Help
										icon="info"
										tooltip={
											<JDlist
												...
												contents={[
													`${LANG('expire_date')}: ${to4YMMDD(applyedProduct.expireDate)}`,
							...
							<div className="JDsideNav__billing-detail">
								<span>{applyedProduct && `${applyedProduct.daysLeftToExpire}${LANG('date')} ${LANG('available')}`}</span>
							</div>
						</div>
				...
				{/* 사이드 네비 외 가림막 */}
				<div role="presentation" onClick={handleCurtainClick} className="JDsideNav-curtain" />
			</div>
