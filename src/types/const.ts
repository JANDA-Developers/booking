...
export const LANGUAGE_LIST: Language[] = [ Language.CHINESE, Language.ENGLISH, Language.JAPANESE, Language.KOREAN ];
...
export const WeekArrEn = [ Day.SUN, Day.MON, Day.TUE, Day.WED, Day.THU, Day.FRI, Day.SAT ];
...
// 위아래 인덱스가 맞아야함
export const SmsReplaceKeyEnumValues = SmsReplaceKeyEnumKeys.map((key) => `%${key}%`);
...
export let PRICING_TYPE_OP = [ { value: PricingType.DOMITORY, label: '' }, { value: PricingType.ROOM, label: '' } ];
...
export let CHECK_IN_OUT_OP = [ { value: true, label: '체크인' }, { value: false, label: '체크아웃' } ];

export let GENDER_OP = [ { value: Gender.FEMALE, label: '' }, { value: Gender.MALE, label: '' } ];
...
export const STATIC_COLORS = [ '#4c5b73', '#E8554E', '#F19C65', '#FFD265', '#2AA876', '#0A7B83' ];
