...
import { updateHouseVariables, updateHouse, UpdateHouseInput } from '../../../types/api';
...
const HouseConfigWrap: React.FC<IProps> = ({ context }) => {
	const [ updateHouseMu, { loading } ] = useMutation<updateHouse, updateHouseVariables>(UPDATE_HOUSE, {
		client,
		onCompleted: ({ UpdateHouse }) => {
			onCompletedMessage(UpdateHouse, LANG('update_house_completed'), LANG('update_house_failed'));
		}
	});
