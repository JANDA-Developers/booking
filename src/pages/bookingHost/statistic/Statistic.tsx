...
const Statistic: React.FC<IProps & WindowSizeProps> = ({ context, loading, staticsWrapProps, windowWidth }) => {
	const { setQueryOp, queryOp, staticData, queryDateHook } = staticsWrapProps;
	const [ viewMode, setViewMode ] = useState<IGraphViewMode>(IGraphViewMode.doughnut);
	...
	const labels = ((): string[] => {
		...
		if (queryOp.unit === SalesStatisticsUnit.BY_DATE)
			return staticData.map((data) => `${data.dateInfo.year}-${data.dateInfo.month}-${data.dateInfo.date}`);

		if (queryOp.unit === SalesStatisticsUnit.MONTHLY)
			return staticData.map((data) => `${data.dateInfo.year}-${data.dateInfo.month}`);

		if (queryOp.unit === SalesStatisticsUnit.WEEKLY)
			return staticData.map((data) => `${data.dateInfo.year}-${data.dateInfo.month}`);

		if (queryOp.unit === SalesStatisticsUnit.YEARLY) return staticData.map((data) => `${data.dateInfo.year}`);
		return [ '' ];
	})();
	...
	const handleTodaySalesStatic = () => {
		...
		queryDateHook.setTo(moment(new Date()).add(1, 'day').toDate());
	};
			...
			<PageBody>
				...
				<div className="flex-grid">
					<div className="flex-grid__col col--full-6 col--wmd-12">
						<Card mb="normal" className="statistic__graphCard">
							{loading ? (
								<div className="statistic__graphPreloader">
									<Preloader wrapClassName="" position="center" noAnimation size={'large'} loading={loading} />
								</div>
							) : (
								<Fragment>
									{/* 통계 아이콘랩 */}
									{!isTabletDown && <StaticIcons context={context} viewMode={viewMode} setViewMode={setViewMode} />}
									{/* 그래프 랩 */}
									<div className={(viewMode === 'list' ? 'statistic__table' : undefined) + ' statistic__graphWrap'}>
										<JDgraph originalData={staticData} JDtype={viewMode} data={graphData} />
									</div>
								</Fragment>
							)}
						</Card>
					...
					{windowWidth > WindowSize.TABLET ? (
						...
						<StaticsControllerModal context={context} staticsProps={staticsProps} modalHook={staticControllerModalHook} />
					)}
				</div>
				{windowWidth > WindowSize.TABLET || (
					<Button
