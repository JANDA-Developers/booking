...
const PeriodicalPayTable: React.FC<Iprops> = ({ historyData, pageInfo, setPage }) => {
	// TODO: 아래세줄들을 hook으로 치환
	const checkBoxTableHook = useCheckBoxTable([], historyData.map((data) => data._id));
	...
	const TableColumns: JDcolumn<IPayHistroy>[] = [
		...
		},
		{
			...
			accessor: 'createdAt',
			Cell: ({ value }) => {
				return <div>{moment(value).local().format(DateFormat.YYMMDD)}</div>;
			}
		}
		// {
		//   // 영수증
