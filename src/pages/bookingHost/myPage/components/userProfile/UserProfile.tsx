...
import { updateMyProfileVariables, updateMyProfile, getUserForSU_GetUserForSU_user } from '../../../../../types/api';
import InputText from '../../../../../atoms/forms/inputText/InputText';
import { isName, isEmail, isPhone } from '../../../../../utils/inputValidations';
import JDIcon from '../../../../../atoms/icons/Icons';
import { IMG_REPO, IS_MOBILE } from '../../../../../types/const';
...
const UserProfile: React.FC<Iprops> = ({ context, userInfo }) => {
	...
	const isPhabletDwon = window.innerWidth < WindowSize.PHABLET || IS_MOBILE;

	const porifleInputs = [
		[
			{
				label: LANG('name'),
				Component: () => <InputText id="UserProfileNameInput" mb="no" {...nameHook} validation={isName} />
			},
			{
				label: LANG('phoneNumber'),
				Component: () => (
					<InputText id="UserProfilePhoneNumberInput" mb="no" hyphen {...phoneNumberHook} validation={isPhone} />
				)
			}
		],
		[
			{
				label: LANG('email'),
				Component: () => <InputText id="UserEmailNumberInput" mb="no" {...emailHook} validation={isEmail} />
			},
			{
	...
	return (
		<Fragment>
			<Card mr="no">
				<CardHeader desc={LANG('mypage_profile_desc')} title={LANG('user_info')} />
				<CardSection>
					<div className="flex-grid">
						<div className="JDstandard-margin-bottom flex-grid__col col--full-12 col--wlg-12">
							<div className="myPage__profileSectionWrap flex-grid-grow">
								...
								<div id="UserProfileVtable" className="formWrap flex-grid__col ">
									<Vtable cellColumn={isPhabletDwon} mode={isPhabletDwon ? 'unStyle' : undefined} border={'none'} mb="no">
					...
					<div className="flex-grid myPage__myHouses">
						{houses &&
							houses.map((house, index) => {
								const commonClasses = 'myPage__myHouse flex-grid__col col--full-3 col--md-12';
								return (
									<Fragment key={house._id}>
										<div className={commonClasses}>
											<MyHouse
												...
												productId={house.product?._id}
												productName={house.product?.name}
										...
										{houses.length === index + 1 && (
											<div
												onClick={() => {
													location.href = insideRedirect('createHouse');
												}}
												className={commonClasses}
											>
												<JDbox clickable mode="border" className="myHouseCard JDflex JDflex--vCenter JDflex--center">
			...
			<PasswordCheckModal modalHook={passwordCheckModalHook} handleCallBackConfirm={handleCallBackConfirm} />
		</Fragment>
	);
};
