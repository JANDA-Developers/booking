...
interface IProps {
	handleTerminateBtn: () => void;
	modalHook: IUseModal;
}

const TerminateModal: React.FC<IProps> = ({ modalHook, handleTerminateBtn }) => {
	return (
		<JDmodal className="terminateModal JDtext-align-center" {...modalHook}>
			<div className="modal__section terminateModal__headWrap">
				...
				<span className="JDsmall-text">{LANG('if_you_cancel_service_please_notice_below_things')}</span>
			</div>
			<div className="modal__section terminateModal__headWrap">
				<JDlist
					mr="no"
			...
			<ModalEndSection>
				<Button thema={'error'} mode="flat" label={LANG('auto_pay_stop')} onClick={handleTerminateBtn} />
			</ModalEndSection>
		</JDmodal>
	);
};
